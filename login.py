...
if st.session_state.is_logged_in:
    ...
    st.write("You are now logged in. You can integrate this with your PathPilot app.")
    if st.button("Logout"):
        st.session_state.is_logged_in = False
        st.session_state.current_user = ""
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("## 🌐 Text-to-Speech Language")
    language_map = {
        "English": "en",
        "Hindi": "hi",
        "Spanish": "es",
    ...
    selected_lang = st.selectbox("Choose language:", list(language_map.keys()), index=0)
    lang_code = language_map[selected_lang]


    if resource_option != "None":
        st.markdown(resource_content.get(resource_option, ""))
    st.markdown("## 📊 Smart Skill Gap Visualizer")
    with st.sidebar.expander("📊 Smart Skill Gap Visualizer", expanded=False):
            st.markdown("Compare your resume skills with target job role skills.")
            resume_skills_input = st.text_area("✍️ Enter Resume Skills (comma-separated):")
            target_skills_input = st.text_area("🎯 Enter Target Role Skills (comma-separated):")
            if st.button("🧠 Visualize Skill Gap"):
                if resume_skills_input.strip() and target_skills_input.strip():
                    resume_skills = [s.strip().lower() for s in resume_skills_input.split(",")]
                    target_skills = [s.strip().lower() for s in target_skills_input.split(",")]

                    all_skills = list(set(resume_skills + target_skills))
                    resume_vector = [1 if skill in resume_skills else 0 for skill in all_skills]
                    target_vector = [1 if skill in target_skills else 0 for skill in all_skills]
                    fig = go.Figure()
                    fig.add_trace(go.Scatterpolar(r=resume_vector, theta=all_skills, fill='toself', name='Your Resume'))
                    fig.add_trace(go.Scatterpolar(r=target_vector, theta=all_skills, fill='toself', name='Target Role'))
                    ...
                    st.plotly_chart(fig)
                    gap_skills = [skill for skill in all_skills if skill in target_skills and skill not in resume_skills]
                    strong_skills = [skill for skill in all_skills if skill in resume_skills and skill in target_skills]
                    extra_skills = [skill for skill in all_skills if skill in resume_skills and skill not in target_skills]

                    explanation = f"### 📊 Skill Gap Insights\n"
                    if strong_skills:
                        explanation += f"✅ **Strong Skills:** {', '.join(strong_skills)}\n\n"
                    if gap_skills:
                        explanation += f"🔍 **Missing or Weak Skills:** {', '.join(gap_skills)}\n\n"
                        explanation += f"📈 **Suggestions to Improve:**\n"
                        for skill in gap_skills:
                            explanation += f"- Learn or improve **{skill}**.\n"
                 # --- Skill Gap Explanation and Learning Paths ---
                            missing_skills = list(set(target_skills) - set(resume_skills))
                            if missing_skills:
                                st.markdown("### 🔍 Skill Gap Explanation")
                                explanation = "The radar chart shows that you currently lack the following key skills required for your target role:\n\n"
                                explanation += "\n".join([f"- {skill.title()}" for skill in missing_skills])
                                explanation += "\n\nTo improve your chances, focus on learning these skills."
                                st.markdown(explanation)
                                st.markdown("### 🎓 Suggested Learning Resources")
                                for skill in missing_skills:
                                    skill_query = skill.replace(" ", "+")
                                    st.markdown(f"**📌 {skill.title()}**")
                                    st.markdown(f"- [YouTube Tutorials](https://www.youtube.com/results?search_query=learn+{skill_query})")
                                    st.markdown(f"- [Coursera Courses](https://www.coursera.org/search?query={skill_query})")
                                    st.markdown(f"- [GeeksforGeeks](https://www.geeksforgeeks.org/?s={skill_query})")
                                    st.markdown(f"- [Udemy Courses](https://www.udemy.com/courses/search/?q={skill_query})")
                                    st.markdown("---")
                            else:
                                    st.success("✅ Great! Your resume already covers all required target skills.")
                            if extra_skills:
                                    explanation += f"\n💡 **Bonus Skills (not in target role but present in resume):** {', '.join(extra_skills)}"
                                    st.markdown(explanation)
                    else:
                        st.warning("⚠️ Please enter both resume and target skills.")
...
if 'chat_history' in st.session_state:
    for i, entry in enumerate(st.session_state.chat_history):
        with st.container():
            ...
            st.markdown(f"<b>🤖 PathPilot:</b> {bot_response}</div>", unsafe_allow_html=True)

            # --- Text-to-Speech Output ---
        listen_key = f"listen_{i}"
        if st.button("🔊 Listen", key=listen_key):
            translated_text = translate_text(bot_response, lang_code)
        ...
        # HTML + JS for custom audio player with playback speed control
            st.markdown(f"""
           <audio id="{audio_id}" controls style="width: 100%; margin-top: 10px;">
               <source src="data:audio/mp3;base64,{b64}" type="audio/mp3">
            </audio>
            <label for="speed_{audio_id}"><b>🎵 Playback Speed:</b></label>
            <select id="speed_{audio_id}" onchange="document.getElementById('{audio_id}').playbackRate = this.value">
        ...
        if entry['feedback'] is None:
            ...
            st.markdown("How did we do? Rate us and drop your thoughts! ⭐📝")
            col_fb = st.columns(5)
            feedback_emojis = ["😀", "😊", "😐", "😕", "😡"]
